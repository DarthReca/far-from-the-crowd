...
from trainer import CurriculumMoCoTask

CHECKPOINT_FOLDER = ""


@hydra.main(
    config_path="configs", config_name="mocov2_curriculum", version_base=None
)
def main(config: OmegaConf) -> None:
    ...
    task = CurriculumMoCoTask(**config.moco)

    logger = CometLogger(project_name="CL-SSL")
    os.makedirs(
        f"{CHECKPOINT_FOLDER}/{logger.experiment.get_key()}", exist_ok=True
    )
    checkpointer = ModelCheckpoint(
        f"{CHECKPOINT_FOLDER}/{logger.experiment.get_key()}",
